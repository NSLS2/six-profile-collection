...
class XIPPlugin(PluginBase):
    ...
    sum_3x3_threshold_min = Cpt(SignalWithRBV, 'SUM_3X3_THRESHOLD_MINIMUM')
    sum_3x3_threshold_max = Cpt(SignalWithRBV, 'SUM_3X3_THRESHOLD_MAXIMUM')

    hist_start = Cpt(SignalWithRBV, 'HISTOGRAM_RANGE_MINIMUM')
    hist_bin_width = Cpt(SignalWithRBV, 'HISTOGRAM_BINWIDTH')
    hist_bin_count = Cpt(SignalWithRBV, 'HISTOGRAM_BINCOUNT')

    source_region = Cpt(EpicsSignal, 'ENABLE _SOURCE_REGION', string=True)
    ...
    isolinear_threshold = Cpt(SignalWithRBV, 'HR_ISOLINEAR_CALIBRATION_THRESHOLD')
